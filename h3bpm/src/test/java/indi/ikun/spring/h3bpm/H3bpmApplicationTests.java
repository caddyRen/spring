...
public class H3bpmApplicationTests {
    ...
    public void testRestTemplate(){
        JSONObject j1=h3bpmHttpUtils.getFlow("ProjectLeader");
    }
...
//       2.项目经理接收派单，先设置现场负责人（现场施工参与者）再审批通过
        ...
        for(int i=0;i<uFinishWork.size();i++){
            if ("项目施工业务流程".equals(uFinishWork.getJSONObject(i).getString("WorkflowName"))
                    &&"接收派单".equals(uFinishWork.getJSONObject(i).getString("DisplayName"))) {
...
//        JSONObject j33=h3bpmHttpUtils.setItemValue(instanceId,m3);
        ...
        for(int i=0;i<uFinishWork3.size();i++){
            if ("项目施工业务流程".equals(uFinishWork3.getJSONObject(i).getString("WorkflowName"))
                    &&"现场施工".equals(uFinishWork3.getJSONObject(i).getString("DisplayName"))) {
       ...
       //4.项目经理申请结项：先设置监理审批人，再审批通过
        ...
        for(int i=0;i<uFinishWork4.size();i++){
            if ("项目施工业务流程".equals(uFinishWork4.getJSONObject(i).getString("WorkflowName"))
                    &&"申请结项".equals(uFinishWork4.getJSONObject(i).getString("DisplayName"))) {
...
//        JSONObject j555=h3bpmHttpUtils.setItemValue(instanceId,"ojbk","spyj_jianli");
        ...
        for(int i=0;i<uFinishWork5.size();i++){
            if ("项目施工业务流程".equals(uFinishWork5.getJSONObject(i).getString("WorkflowName"))
                    &&"监理审批".equals(uFinishWork5.getJSONObject(i).getString("DisplayName"))) {
...
//        JSONObject j66=h3bpmHttpUtils.setItemValue(instanceId,"ojbk","spyj_bzsp");
        ...
        for(int i=0;i<uFinishWork6.size();i++){
            if ("项目施工业务流程".equals(uFinishWork6.getJSONObject(i).getString("WorkflowName"))
                    &&"班组验收负责人审批".equals(uFinishWork6.getJSONObject(i).getString("DisplayName"))) {
...
//        JSONObject j77=h3bpmHttpUtils.setItemValue(instanceId,"ojbk","spyj_fzr");
        ...
        for(int i=0;i<uFinishWork7.size();i++){
            if ("项目施工业务流程".equals(uFinishWork7.getJSONObject(i).getString("WorkflowName"))
                    &&"项目负责人审批".equals(uFinishWork7.getJSONObject(i).getString("DisplayName"))) {
