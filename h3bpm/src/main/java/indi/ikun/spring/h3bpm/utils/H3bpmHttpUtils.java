...
/**
 * @TODO //项目施工业务流程 1.发起流程，此时设置好需要设置好的参与者：项目负责人（对应：项目负责人审批），项目经理（对应：接收派单，申请结项） 2.项目经理接收派单，先设置现场负责人（现场施工参与者）再审批通过
 *       3.现场负责人，现场施工直接审批通过 4.项目经理申请结项：先设置监理审批人，再审批通过 5.监理审理：先设置班组验收，再审批通过 6.班组验收，直接审批通过 7.项目负责人审批通过，
 * @Author: renqiankun iIlL1
...
public class H3bpmHttpUtils {
    ...
    public JSONObject startFlow(String Code, String userCode, Map<String, String> map) {
        ...
        for (String str : xmsgywlcMap(1).keySet()) {
            ...
            for (String str2 : map.keySet()) {
                if (str.equals(str2)) {
                    try {
                        if (str2.equals("beginDate") || str2.equals("endDate") || str2.equals("pdDate")
                            || str2.equals("planEndTime")) {
        ...
        try {
            ResponseEntity<String> responseEntity = restTemplate.exchange(url.toString(), HttpMethod.POST,
                getHttpEntityJSON(MediaType.APPLICATION_JSON_UTF8, params.toJSONString()), String.class);
            if (responseEntity.hasBody()) {
                String decodeResult =
                    H3bpmHttpUtils.decrypt(responseEntity.getBody(), RSAUtil.getPrivateKey(privateKey));
                log.info("h3bpm接口数据 status code ======>" + responseEntity.getStatusCodeValue());
                log.info("h3bpm接口数据 body        ======>" + decodeResult);
                log.info("h3bpm接口数据 Headers     ======>" + responseEntity.getHeaders());
                JSONObject result = JSON.parseObject(decodeResult);
     ...
     * @param:
     * @return: userId":"d68f8e44-eccd-426b-b3b9-94ffe8c34213", "workItemId":"782c9936-7c59-457c-9182-341b441e23cc"
     *          "commentText":"同意"
     **/
    public JSONObject sptg(String userName, String workItemId) {
        ...
        log.info("======>请求参数：" + params.toJSONString());
        try {
            ResponseEntity<String> responseEntity = restTemplate.exchange(url.toString(), HttpMethod.PUT,
                getHttpEntityJSON(MediaType.APPLICATION_JSON_UTF8, params.toJSONString()), String.class);
            if (responseEntity.hasBody()) {
                String decodeResult =
                    H3bpmHttpUtils.decrypt(responseEntity.getBody(), RSAUtil.getPrivateKey(privateKey));
     ...
     * @param: 指定指定节点参与者
     * @return: { "systemCode": "H3", "secret": "Authine", "userId": "18f923a7-5a5e-426d-94ae-a55ad1a4b239",
     *          "bizObjectSchemaCode": "TD", "bizObjectId": "52491396-d2cc-423e-8526-62df1ad1eae6", "keyName":
     *          "message", "keyValue": "Hello, H3 BPM!" }
     **/
    ...
    /**
     * @TODO // HttpEntity<MultiValueMap<String, String>>
     * @Author:renqiankun
     * @Date:2019-12-10 18:18
     * @param:
     * @return:
     **/
    private HttpEntity<MultiValueMap<String, Object>> getHttpEntity(MediaType mediaType,
     ...
     *             h3系统用户名，4a
     * @return: 返回当前用户id
     *          usserid?systemCode=H3&secret=Authine&startTime=2019-01-21&endTime=2019-02-01&startIndex=1&endIndex=1
     **/
    public JSONObject getUnFinishWork(String userName) {
        ...
        log.info("======>发起请求：" + url.toString());
        try {
            JSONObject result = JSON.parseObject(H3bpmHttpUtils
                .decrypt(restTemplate.getForObject(url.toString(), String.class), RSAUtil.getPrivateKey(privateKey)));
            log.info("======>h3bpm返回结果：" + result.toJSONString());
            // 获取Code，DisplayName，ObjectID
            if (0 == result.getInteger("code")) {
                // httpCode
                jsonObject.put("code", 0);
